...
main(int argc, char* argv[])
{
	...
	/* Allocating memory */
	hostMalloc(&first,  (dim_ * dim_) * sizeof(int32_t));
	hostMalloc(&second, (dim_ * dim_) * sizeof(int32_t));
	hostMalloc(&output, (dim_ * dim_) * sizeof(int32_t));
	memset(output, 0, (dim_ * dim_));
	/* Initialization. */
	srand(time(NULL));
